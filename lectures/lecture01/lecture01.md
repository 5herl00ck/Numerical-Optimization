...
9. Обзор курса, инструменты, литература
10. Упражнения

Конспект опирается на главу 1 лекционных заметок М. Диля (*Lecture Notes on Numerical Optimization*, 2016) и главу 1 книги Nocedal & Wright. Код демонстраций — [`demo01.ipynb`](demo01.ipynb) (пошаговый ноутбук с пояснениями по `scipy.optimize` и стандартной форме каждого примера) и [`demo01.py`](demo01.py) (те же примеры компактным скриптом). Картинки строятся скриптом [`make_figures.py`](make_figures.py).

---

## 1. Зачем это вам: три задачи

Прежде чем давать определения, посмотрим на три задачи, которые будут сопровождать нас весь курс.
...
Вспомогательные переменные — главный инструмент моделирования, и мы ещё увидим его в деле в разделе 8: они позволяют, например, избавиться от модуля.
...
| Задача наименьших квадратов (LS) | $\tfrac12\Vert r(x)\Vert^2$ | обычно нет |
...
**Моделирование влияет на трудность.** Рассмотрим задачу $\min_x \sum_i |a_i^\top x - b_i|$ (подгонка в норме $\ell_1$, устойчивая к выбросам). Функция негладкая, и методы курса напрямую неприменимы. Введём переменные $s_i \ge |a_i^\top x - b_i|$ (картинка в разделе 3.1):
...
Для разбора на занятии (после лекции) и самостоятельно. Подробные решения — в [`exercises01.md`](exercises01.md).

**10.1.** Запишите в стандартной форме (NLP) и определите класс:
...
**10.2.** Приведите пример задачи, у которой (а) допустимое множество непусто и ограничено, но минимума нет; (б) бесконечно много глобальных минимумов; (в) есть ровно один локальный минимум, но нет глобального.

**10.3.** Для функции $f(x) = \tfrac12 x^\top Q x + c^\top x$ с симметричной $Q$ выпишите $\nabla f$ и $\nabla^2 f$. Когда $\arg\min f$ — единственная точка?

**10.4.** В примере 7.2 решите LP графически: нарисуйте допустимый многоугольник и линии уровня целевой функции. Убедитесь, что решение $(2, 6)$ — вершина. Какие ограничения активны? Что изменится, если прибыль от первого продукта вырастет до 10?

**10.5.** Запустите `demo01.py`, часть (d). Объясните, почему запуски из разных начальных точек дают разные ответы. Как найти глобальный минимум в одномерном случае? Почему этот приём не масштабируется на $n = 100$?
