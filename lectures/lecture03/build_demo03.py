...
- **(b)** двойственность QP на ящике из демо к лекции 2: максимизируем $d(\mu)$ численно, сверяем с уже известным решением, комплементарная нежёсткость;
- **(c)** двойственность LP на двух старых знакомых — планировании производства (лекция 1) и диете (ДЗ 1): `res.ineqlin.marginals` против явного решения двойственной задачи;
- **(d)** когда двойственности нельзя доверять — невыпуклый пример с бесконечным зазором и «скрытая выпуклость» задачи из ДЗ 2.
