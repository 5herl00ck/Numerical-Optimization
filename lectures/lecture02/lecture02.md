...
2. **Почему** для выпуклой задачи любой локальный минимум глобален — единственная теорема лекции, и мы её докажем целиком.
...
Всё остальное в курсе — методы — будет опираться на эти три ответа.
...
| эллипсоид | $\{x : (x-c)^\top Q (x - c) \le 1\}$, $Q \succ 0$ | аффинный образ шара |
...
| $e^{x}$, $x^2$, $x^4$, $\lvert x \rvert$ | $\mathbb{R}$ | по определению или по второй производной |
...
- Квадратичная $f(x) = \tfrac12 x^\top Q x + c^\top x$: гессиан $\nabla^2 f = Q$ не зависит от $x$, поэтому $f$ выпукла $\iff Q \succeq 0$, строго выпукла $\iff Q \succ 0$. Практическая проверка — `np.linalg.eigvalsh(Q).min() >= 0`.
...
Критерий второго порядка требует гессиана; часто проще собрать функцию из выпуклых «кирпичей» операциями, которые выпуклость сохраняют.
...
**Классы выпуклых задач.** LP (всё аффинно) $\subset$ выпуклая QP ($Q \succeq 0$, ограничения аффинные) $\subset$ выпуклая QCQP (цель и все $f_i$ — выпуклые квадратичные). Все они — частные случаи выпуклой NLP, и для каждого есть специализированные солверы, которые мы будем изучать в части IV.
...
3. Неравенства $h_i(x) \ge 0$ — с вогнутыми $h_i$: аффинные, $r^2 - \Vert x - c\Vert^2$, $\log x_i$; **не** $\Vert x\Vert^2 - r^2 \ge 0$ (внешность шара) и не $x_1 x_2 \ge 1$ (проверьте!).
...
**Теорема.** Пусть $f \in C^1$ выпукла на выпуклом $\Omega$. Точка $x^\ast \in \Omega$ — глобальное решение задачи $\min_{x \in \Omega} f(x)$ тогда и только тогда, когда
...
**Пример 1: квадратичная функция.** Для $f(x) = \tfrac12 x^\top B x + c^\top x$ с $B \succ 0$ условие $\nabla f = Bx + c = 0$ даёт единственное решение $x^\ast = -B^{-1} c$, а оптимальное значение

$$
\min_x \Big(\tfrac12 x^\top B x + c^\top x\Big) = -\tfrac12\, c^\top B^{-1} c .
$$

Эта формула будет постоянно появляться в части II: метод Ньютона на каждом шаге решает именно такую задачу.
...
**Лекция 3 — двойственность.** Функция Лагранжа $L(x, \lambda, \mu) = f(x) - \lambda^\top g(x) - \mu^\top h(x)$, двойственная функция как нижняя граница $f^\ast$, слабая и сильная двойственность, условие Слейтера, двойственные задачи для LP и QP. Двойственность — второй способ (после условия раздела 7) сертифицировать решение выпуклой задачи и источник множителей Лагранжа для всей части III.

**Домашнее задание 2** выдаётся 23 сентября (тема — выпуклость и двойственность), срок — 7 октября.
...
**Домашнее задание 2** выдаётся 23 сентября.
